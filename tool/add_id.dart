library bwu_emojione_iconset_svg.tool.add_id;

import 'dart:io' as io;
import 'dart:async' as async;
import 'package:path/path.dart' as path;
import 'src/emojione.dart';

// see http://apps.timwhitlock.info/emoji/tables/unicode for details
Map categories = {
    'emoticons': [0x1F600, 0x1F64F],
    'dingbats': [0x2702, 0x27B0],
    'transport_and_map': [0x1F680,0x1F6C5],
    'enclosed_characters': [0x24C2,0x1F251],
    'uncategorized': [-1, -1],
};

String getCategory(String id) {
  var unicodes = id.split('-');
  for(var s in unicodes) {
    var val = int.parse(s, radix: 16);
    for(String k in categories.keys) {
      if(val >= categories[k][0] && val <= categories[k][1]) {
        return k;
      }
    }
  };
...
void main() {
  ...
  var svgOutDir = new io.Directory('lib/assets/svg');
  var svgInDir = new io.Directory('lib/assets/svg_bak');
  List<String> ids = [];
  svgInDir.list(followLinks:false)
  .listen((f) {
    if (f is io.File && path.extension(f.path) == '.svg') {
      var inFile = f as io.File;
      String svg = inFile.readAsStringSync();
      var id = path.basenameWithoutExtension(f.path).toUpperCase();
      //print('Unicode: ${id} - image: ${unicodeToImage(id)}');
      ids.add(id);
      svg = svg.replaceFirst('<svg ', '<svg id="U${id}" ');
      new io.File(path.join(svgOutDir.path, path.basename(inFile.path))).openWrite()
        ..write(svg)
        ..flush().then((ioSink) => ioSink.close());
    } else {
      print('Ignoreing: ${f.path}');
    }
  }, onDone: () {
    Map idList = <String,io.IOSink>{};
    categories.keys.forEach((k) {
      idList[k] = new io.File('lib/assets/id_list_${k}.txt').openWrite();
    });

    ids.forEach((id) {
      idList[getCategory(id)].write(def(id));
    });

    ascii = false;
    emojioneList.forEach((k, v) {
      //print('Shortname: ${k} - image: ${shortnameToImage(k)}');
      var found = false;
      //v.forEach((id) {
      var id = v.last.toUpperCase();
      if (ids.contains(id)) {
        idList[getCategory(id)].write(def2(id, k.substring(1, k.length - 1)));
        found = true;
      }
      if(!found) print('Ignoring: ${k} - ${id}');
      //});
    });

//    ascii = true;
//    asciiList.forEach((k, v) {
//      //print('ASCII: ${k} - image: ${unicodeToImage(k)}');
//      var found = false;
//      var id = v.toUpperCase();
//      if (ids.contains(id)) {
//        idList[getCategory(id)].write(def2(id, escapeHTML(k)));
//        found = true;
//      }
//      if(!found) print('Ignoring: ${k} - ${id}');
//    });
    var flushOps = <async.Future>[];
    idList.forEach((k, io.IOSink v) {
      flushOps.add(v.flush()
      .then((ioSink) => ioSink.close()));
    });
    return async.Future.wait(flushOps);
  });
}
...
String def2(String id, String newId) {
  if (newId == null) {
    newId = id;
  }
  return '''
